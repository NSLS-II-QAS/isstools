...
class UIBatchManual(*uic.loadUiType(ui_path)):
    ...
    def load_samples(self):
        ...
        if filename:
            ...
            for sample in samples:
                _create_new_sample(sample['name'], sample['comment'], sample['x'], sample['y'],
    ...
    def populate_service_parameters(self, index):
        ...
        for i in range(len(self.service_parameter_values)):
            ...
            self.gridLayout_service_parameters.addWidget(self.service_parameter_descriptions[i], i, 1,
                                                         QtCore.Qt.AlignTop)
                ...
                for index in range(self.listView_scans.model().rowCount()):
                    ...
                    if item_scan.checkState():
                        ...
                        if self.radioButton_sample_map_1D.isChecked():
                            x_coord = np.linspace(self.spinBox_sample_x_map_start.value(),
                                                  self.spinBox_sample_x_map_end.value(),
                                                  int(self.spinBox_sample_x_map_steps.value()))
                            y_coord = np.linspace(self.spinBox_sample_y_map_start.value(),
                            ...
                            y_points = np.linspace(self.spinBox_sample_y_map_start.value(),
                                                   self.spinBox_sample_y_map_end.value(),
                            ...
                            for i in range(int(self.spinBox_sample_y_map_steps.value())):
                                x_line = np.linspace(self.spinBox_sample_x_map_start.value(),
                                                     self.spinBox_sample_x_map_end.value(),
                                                     int(self.spinBox_sample_x_map_steps.value()))
