...
class BatchManager():
    ...
    def load_csv(self, filepath):
        ...
        for name, columns in [[name, tables[name].columns[-1::-1]] for name in [table for table in ['Create Scans']]]:
            for column in columns:
                if tables[name][[column]].isnull().get()s.all():
                    tables[name] = tables[name].drop(
                        tables[name].columns[[tables['Create Scans'].columns.get_loc(column)]], axis=1)
    ...
    def save_csv(self, filepath):
        ...
        for text in data_samples:
            ...
            values.append(collections.OrderedDict((k.strip(), v.strip()) for k, v in
                                                  (item.split(':') for item in text[text.find(' X:') + 1:].split(' '))))
        ...
        for text in data_scans:
            ...
            values.append(collections.OrderedDict((k.strip(), v.strip()) for k, v in (item.split(':') for item in text[
                                                                                                                  text.rfind(
                                                                                                                      ' ',
                                                                                                                      0,
                                                                                                                      text.find(
        ...
        tables = dict(
            {'Create Samples': csamp, 'Create Scans': cscans, 'Create Sample Loops': csloop, 'Batch Mode': batch})
